...
import {
  ...
  useDismissAlert,
  type FinancialAlert,
} from "@/modules/finance-erp/hooks/useFinancialAlerts";
...
export function ERPAlertsPage() {
  ...
  const unreadAlerts: FinancialAlert[] = alerts.filter((a: FinancialAlert) => !a.is_read);
  const readAlerts: FinancialAlert[] = alerts.filter((a: FinancialAlert) => a.is_read);
  ...
  const AlertCard = ({ alert }: { alert: FinancialAlert }) => (
    <Card className={`panel ${!alert.is_read ? "border-l-4 border-l-primary" : ""}`}>
