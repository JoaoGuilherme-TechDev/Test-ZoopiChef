...
import { CalendarIcon, Calendar as CalendarIcon2, Loader2, Package, ClipboardList, Printer, FileText } from 'lucide-react';
...
export default function ScheduledOrdersReport() {
  const { company } = useCompanyContext();
  const [selectedDate, setSelectedDate] = useState<Date>(new Date());

  // Fetch scheduled orders for the selected date
  const { data: orders, isLoading } = useQuery({
    queryKey: ['scheduled-orders-report', company?.id, selectedDate?.toISOString().split('T')[0]],
    ...
    queryFn: async () => {
      ...
      const { data: ordersData, error: ordersError } = await supabase
        ...
        .in('status', ['confirmed', 'pending']) // Only active orders
  ...
  const productSummary = useMemo(() => {
    ...
    orders.forEach(order => {
      (order.items || []).forEach(item => {
        const key = item.product_id || item.product_name; // Use name as fallback key
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-display font-bold">Relatório de Produção Agendada</h1>
