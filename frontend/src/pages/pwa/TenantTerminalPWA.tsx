/**
 * TenantTerminalPWA - Unified PWA entry point for Terminal do Operador
 * 
 * Route: /:slug/terminal
 * 
 * Flow:
 ...
 * 3. Require login (operator must be authenticated)
 * 4. Validate user has terminal/operator permissions
 * 5. Load Operator Terminal
 */
...
// Lazy load the actual Operator Terminal
...
export default function TenantTerminalPWA() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (session?.user) {
        // Verify user belongs to this restaurant and has operator role
        if (restaurant) {
          ...
          if (profile?.company_id === restaurant.companyId && 
              ['operador', 'admin', 'gerente'].includes(userRole)) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!['operador', 'admin', 'gerente'].includes(userRole)) {
        await supabase.auth.signOut();
        toast.error('Você não tem permissão para acessar o Terminal do Operador');
        return;
      }
          ...
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              ...
              <CardDescription className="text-base mt-2">
                Terminal do Operador - Faça login para continuar
              </CardDescription>
            </div>
          </CardHeader>
  ...
  // Authenticated - render Operator Terminal
