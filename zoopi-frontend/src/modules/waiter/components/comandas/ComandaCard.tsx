// ================================================================
// FILE: waiter/components/comandas/ComandaCard.tsx
// Individual comanda card displayed in the comandas grid
// ================================================================

import { useState, useEffect } from "react";
import { Tag, Receipt, Percent } from "lucide-react";
import { cn } from "@/lib/utils";
import { Comanda, COMANDA_STATUS_CONFIG } from "../../types";
...
export function ComandaCard({ comanda, onClick }: ComandaCardProps) {
  const cfg = COMANDA_STATUS_CONFIG[comanda.status];
  const elapsed = useElapsedSeconds(comanda.openedAt);
  const isFree = comanda.status === "free";
  ...
  return (
    ...
    >
      ...
      {comanda.clientName && (
        <p className="text-[11px] font-semibold text-foreground/90 leading-none truncate max-w-full px-1">
          {comanda.clientName}
        </p>
      )}
      ...
      {!isFree && !isClosed && comanda.openedAt && (
        <p className="text-[11px] font-mono text-blue-300 leading-none tabular-nums">
          {formatElapsed(elapsed)}
        </p>
      )}

      {comanda.total !== undefined && !isFree && !isClosed && (
        <p className="text-sm font-extrabold text-blue-300 leading-none">
          R$ {comanda.total.toFixed(2).replace(".", ",")}
        </p>
      )}

      {comanda.serviceCharge !== undefined && !isFree && !isClosed && (
        <p className="text-[10px] text-muted-foreground leading-none flex items-center gap-0.5">
          <Percent className="h-2.5 w-2.5" />
          {comanda.serviceCharge}%
        </p>
      )}

      {isFree || isClosed ? (
        <p className="text-[10px] text-muted-foreground mt-0.5">
          {isClosed ? "Fechada" : "Toque para abrir"}
        </p>
      ) : (
