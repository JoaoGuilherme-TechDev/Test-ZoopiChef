...
import api from '@/lib/api'; // Nosso Axios centralizado
export type PizzaKDSStep = 'prep' | 'oven' | 'assembly' | 'delivery' | 'done';
...
export function PizzaKDSSessionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const validateSession = async () => {
      ...
      try {
        ...
        // Chamada ao seu novo backend NestJS
        const { data } = await api.post('/kds/auth/validate', { action: 'validate_session', token });
  ...
  const validateSlug = useCallback(async (slug: string) => {
    try {
      const { data } = await api.post('/kds/auth/slug', { action: 'validate_slug', slug });
  ...
  const login = useCallback(
    async (slug: string, pin: string) => {
      try {
        const { data } = await api.post('/kds/auth/login', { action: 'login_by_pin', slug, pin });
  ...
  const logout = useCallback(async () => {
    if (state.sessionToken) {
      await api.post('/kds/auth/logout', { action: 'logout', token: state.sessionToken });
    }
