...
import { usePWAWaiterComandasWithCompany, usePWAComandaSettingsWithCompany, PWAComandaStatus } from '@/hooks/usePWAWaiterHooks';
import { WaiterComandaCreateDialog } from '@/components/waiter/WaiterComandaCreateDialog';
import { ArrowLeft, Search, Loader2, Clock, Plus, Tag, RefreshCw } from 'lucide-react';
import { toast } from 'sonner';
import { formatDistanceToNow } from 'date-fns';
import { ptBR } from 'date-fns/locale';

const statusConfig: Record<PWAComandaStatus, { bg: string; border: string; label: string; color: string }> = {
...
export default function PWAWaiterComandasScreen() {
  ...
  const { comandas, isLoading, createComanda } = usePWAWaiterComandasWithCompany(
    companyId,
    ['free', 'open', 'no_activity', 'requested_bill']
  );
  
  // Get comanda settings for service fee default
  const { settings: comandaSettings } = usePWAComandaSettingsWithCompany(companyId);
  ...
  const handleCreateComanda = async (name: string, applyServiceFee: boolean, serviceFeePercent: number) => {
    try {
      ...
      navigate(`/${slug}/garcom/comanda/${newComanda.id}`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 px-4">
        {filteredComandas.map((comanda) => {
          const status = comanda.status as PWAComandaStatus;
          ...
          return (
            <Card
              ...
              onClick={() => navigate(`/${slug}/garcom/comanda/${comanda.id}`)}
            >
              <div className="text-center">
                {/* Comanda Icon */}
