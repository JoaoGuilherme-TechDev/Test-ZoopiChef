...
import { Search, Plus, Minus, Loader2, ShoppingCart, Settings2, Pizza } from 'lucide-react';
import { toast } from 'sonner';

interface WaiterComandaProductSelectorProps {
  open: boolean;
...
export function WaiterComandaProductSelector({
  open,
  onOpenChange,
  comandaId,
  comandaNumber,
  comandaName,
  onSuccess,
}: WaiterComandaProductSelectorProps) {
  ...
  const itemMutations = useComandaItemMutations();
  const { createComandaOrder } = useComandaOrderProduction();

  useEffect(() => {
    // Debug logs to understand why products may not show on mobile
    console.log('[WaiterComandaProductSelector] open=', open);
    console.log('[WaiterComandaProductSelector] categories=', categories.length);
    console.log('[WaiterComandaProductSelector] products=', products.length);
    console.log('[WaiterComandaProductSelector] selectedCategoryId=', selectedCategoryId);
    if (products[0]) console.log('[WaiterComandaProductSelector] sample product=', products[0]);
    if (productsQuery.error) console.log('[WaiterComandaProductSelector] products error=', productsQuery.error);
    if (categoriesQuery.error) console.log('[WaiterComandaProductSelector] categories error=', categoriesQuery.error);
  ...
  // Inner component for pizza price display
  const WaiterProductPrice = ({ product }: { product: { id: string; price: number; subcategory?: any } }) => {
    ...
    if (isPizza) {
      if (isLoading) return <span className="text-sm font-semibold text-primary">Carregando...</span>;
      if (lowestPrice !== null && lowestPrice !== undefined && lowestPrice > 0) {
        return <span className="text-sm font-semibold text-primary">A partir de {formatCurrency(lowestPrice)}</span>;
      }
      return <span className="text-sm font-semibold text-primary">Escolha uma opção</span>;
    }

    return <span className="text-sm font-semibold text-primary">{formatCurrency(product.price)}</span>;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const item of Object.values(cart)) {
        await itemMutations.addItem.mutateAsync({
          comandaId,
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg max-h-[90vh] flex flex-col p-0">
          <DialogHeader className="p-4 border-b">
            <DialogTitle>
              Adicionar Produtos - Comanda #{comandaNumber}
              {comandaName && <span className="text-muted-foreground ml-2">({comandaName})</span>}
            </DialogTitle>
          </DialogHeader>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <WaiterProductPrice product={product} />
                        {hasOptionsSelected && (
