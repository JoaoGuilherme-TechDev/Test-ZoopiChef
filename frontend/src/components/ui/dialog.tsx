...
interface DialogContentProps
  ...
  /** Hide the DialogTitle visually but keep it for accessibility */
  hideTitle?: boolean;
  /** Custom fallback title for accessibility (only used when no DialogTitle provided) */
...
const DialogContent = React.forwardRef<
  ...
  DialogContentProps
>(({ className, children, hideTitle, accessibilityTitle = "Dialog", ...props }, ref) => {
  // Check if children contain a DialogTitle
  const hasDialogTitle = React.Children.toArray(children).some((child) => {
    if (React.isValidElement(child)) {
      // Check direct child or wrapped in DialogHeader
      ...
      if ((child.type as any)?.displayName === "DialogHeader") {
        return React.Children.toArray(child.props.children).some(
          (headerChild) =>
            ...
            (headerChild.type as any)?.displayName === "DialogTitle"
        );
      }
    }
    return false;
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        {...props}
      >
        {/* Provide fallback hidden title for accessibility when not provided */}
        {!hasDialogTitle && (
