...
export function BatchMoveProducts() {
  ...
  const handleConfirm = () => {
    if (!destSubcategoryId) {
      toast.error('Selecione uma subcategoria de destino');
      return;
    }

    batchMove.mutate(
      { productIds: selectedIds, targetSubcategoryId: destSubcategoryId },
      {
        onSuccess: () => {
  ...
  const canProceed = selectedIds.length > 0 && destSubcategoryId && destSubcategoryId !== originSubcategoryId;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
                  ...
                  <SelectContent>
                    {destSubcategories.map(sub => (
                      <SelectItem
                        key={sub.id}
                        value={sub.id}
                        disabled={sub.id === originSubcategoryId}
                      >
                        {sub.name}
                        {sub.id === originSubcategoryId && ' (mesma origem)'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        ) : originSubcategoryId && !isLoading ? (
          <div className="text-center py-8 text-muted-foreground border rounded-lg">
            Nenhum produto encontrado nesta subcategoria
          </div>
        ) : !originSubcategoryId ? (
          <div className="text-center py-8 text-muted-foreground border rounded-lg">
            Selecione uma subcategoria de origem para visualizar produtos
          </div>
        ) : null}
